...
export function DefaultEntityContentLayout(props: EntityContentLayoutProps) {
  const cards = props.cards.splice(0).sort((cardA, cardB) => {
    const prioA = cardA.element.props.node.spec.config?.priority ?? 0;
    const prioB = cardB.element.props.node.spec.config?.priority ?? 0;
    return prioA - prioB;
  });

  console.log('xxx DefaultEntityContentLayout props cards', props.cards);
  console.log('xxx DefaultEntityContentLayout sorted cards', cards);
  console.log('xxx DefaultEntityContentLayout configs', cards.map((card) => card.element.props.node.spec.config));
