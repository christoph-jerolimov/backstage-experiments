...
import { buildFilterFn } from '../catalog/FilterWrapper';
  ...
  factory(originalFactory, { config, inputs }) {
    return originalFactory({
      ...
      loader: async () => {
        ...
        return <Component headers={headers} menuItems={menuItems} groups={groups} />;
      },
    });
  },
});
...
interface ComponentProps {
  headers: {
    ...
    filter: ((entity: Entity) => boolean) | undefined;
  }[];
  menuItems: {
...
const Component = ({ headers, menuItems, groups }: ComponentProps) => {
  console.log('xxx catalogEntityPage groups', groups);
  console.log('xxx catalogEntityPage titles', Object.values(groups).flatMap(group => group.title));
        ...
        {Object.values(groups).flatMap(({ title, items }) =>
          items.map(output => (
            <EntityLayout.Route
              ...
              title={'XXX ' + output.get(EntityContentBlueprint.dataRefs.title)}
              if={buildFilterFn(
                output.get(
