...
import { actionTestPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [actionTestPlugin],
    });

    await request(server).get('/api/action-test/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/action-test/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/action-test/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/action-test/todos/${createdTodoItem.id}`)
